...
![control_room_control_schema.png](assets/control_room_control_schema.png)
...
2. `module 1` implements a primary command for `STARTRECORD` which also accepts a keyword argument for `filename`.

## Use case example Python

Assume we have a `controller` module which needs to switch some stimulation (ON/OFF) via a `stimulator` module - like what is done in the [dp-bollinger-control](https://github.com/bsdlab/dp-bollinger-control).
